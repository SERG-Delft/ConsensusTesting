 ...
 * <p/>
 * After initialization {@link TLSServer#start()} should be called so the server starts to listen to
 ...
 * start the server in its run method and also provide a stop method, which will call {@link TLSServer#stop()}.
 * </p>
 ...
 *
 */
public class TLSServer extends TLSPeer {
     ...
     * @throws Exception
     */
    public TLSServer(String protocol, String hostAddress, int port, String publicKey) throws Exception {
     ...
     * you should use {@link TLSServer#stop()} which will set it to inactive state
    ...
    /**
     * Sets the server to an inactive state, in order to exit the reading loop in {@link TLSServer#start()}
     ...
     * As soon as the server reads these data, it will call {@link TLSServer#write(SocketChannel, SSLEngine, String)}
