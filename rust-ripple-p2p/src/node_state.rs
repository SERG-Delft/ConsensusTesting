...
pub struct NodeStates {
    ...
    pub receive_dependency_graph: Graph<DependencyEvent, ()>,
...
impl NodeStates {
    pub fn new(node_states: Vec<NodeState>) -> Self {
        NodeStates {
            ...
            receive_dependency_graph: petgraph::Graph::new(),
    ...
    pub fn clear_dependency_graph(&mut self) {
        ...
        self.receive_dependency_graph = petgraph::Graph::new();
        for i in 0..self.node_states.len() {
    ...
    pub fn add_receive_receive_dependency(&mut self, ripple_message: RippleMessage) {
        let dependency_event = DependencyEvent { ripple_message: ripple_message.clone(), event_type: EventType::Receive };
        let node_index = self.receive_dependency_graph.add_node(dependency_event.clone());
        let dependency_node = DependencyNode { event: dependency_event, index: node_index };
            ...
            .position(|x| x.0 == ripple_message)
        {
            ...
            match dependency_node {
                None => {}
                Some(node) => { self.receive_dependency_graph.add_edge(node.index, node_index, ()); }
            }
        }

        match receiver_node_state.latest_message_received {
            None => {}
            Some(receive_dependency) => { self.receive_dependency_graph.add_edge(receive_dependency.index, node_index, ()); }
        }
    ...
    pub fn add_receive_send_dependency(&mut self, ripple_message: RippleMessage) {
        let mut sender_node_state = self.node_states[ripple_message.sender_index()].clone();
...
impl MutexNodeStates {
    ...
    pub fn add_execution(&self, ripple_message: RippleMessage) {
        ...
        states.add_receive_receive_dependency(ripple_message.clone());
    }
    ...
    pub fn add_send_dependency(&self, ripple_message: RippleMessage) {
        ...
        self.node_states.lock().add_receive_send_dependency(ripple_message);
    }
    ...
    pub fn get_receive_dependency_graph(&self) -> Graph<DependencyEvent, ()> {
        self.node_states.lock().receive_dependency_graph.clone()
    }
}
