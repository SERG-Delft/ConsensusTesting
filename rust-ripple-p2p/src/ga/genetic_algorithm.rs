...
pub fn run<S, C, T, G, P>(scheduler_sender: Sender<P>, scheduler_receiver: Receiver<T>, params: Parameter<S, C, T, G>, initial_population: Population<G>)
    where S: SelectionOp<G, T> + Debug, C: CrossoverOp<G> + Debug + Sync, T: ExtendedFitness + 'static, G: SuperExtendedGenotype + 'static, P: ExtendedPhenotype<G> + 'static
{
...
pub fn run_permutation_ga<S, T, H>(scheduler_handler: H, fitness_calculation: FitnessCalculation<T, PriorityGenotype>, params: PermutationParameters<S, T, PriorityGenotype>, initial_population: Population<PriorityGenotype>)
...
mod ga_tests {
    ...
    fn test_mu_lambda() {
        ...
        run_ga::<MaximizeSelector, SimulatedBinaryCrossBreeder, CurrentFitness, TestSchedulerHandler, DelaysGenotype, DelayMapPhenotype>(scheduler_handler, fitness_calculation, params, population);
    }
