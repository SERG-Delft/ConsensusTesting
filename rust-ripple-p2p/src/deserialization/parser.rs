...
use nom::branch::alt;
use nom::combinator::{fail, map, map_res, peek, rest, success, value, verify};
use nom::error::{dbg_dmp, Error};
use nom::multi::{length_value, many0};
use nom::number::complete::{be_u16, be_u32, be_u64, be_u8, u64};
use nom::sequence::{pair, preceded, tuple};
use crate::deserialization::mutate_and_serialize_json;
use crate::deserialization::types::{SerializationField, UInt16};
use crate::deserialization::types::SerializationField::U16;
...
fn field_id(input: &[u8]) -> IResult<&[u8], (u8, u8)> {
    // let next_byte = &;
    let (input, (high, low)) = bits::<_, _, Error<(&[u8], usize)>, _, _>(
        pair(bits::complete::take(4usize), bits::complete::take(4usize))
    )(input)?;
    match (high != 0, low != 0) {
        ...
        (false, false) => pair(map(take(1usize), |x: &[u8]| x[0]), map(take(1usize), |x: &[u8]| x[0]))(input)
    }
}
...
fn parse_hash256(input: &[u8]) -> IResult<&[u8], JsonValue> {
    map(take(32usize),
        |hash: &[u8]| JsonValue::String(hex::encode(hash)))(input)
}

fn parse_length_1_byte(input: &[u8]) -> IResult<&[u8], u32> {
    // println!("1b");
    map(verify(be_u8, |n: &u8| {
        // println!("{}", *n);
        *n <= 192
    }), |n: u8| n as u32)(input)
}
...
fn parse_length_3_bytes(input: &[u8]) -> IResult<&[u8], u32> {
    // println!("3b");
    let formula = |(x, y, z): (u8, u8, u8)| 12481 + (((x as u32) - 241) * 65536) + ((y as u32) * 256) + (z as u32);
    map(tuple((be_u8, be_u8, be_u8)), formula)(input)
}

fn parse_length(input: &[u8]) -> IResult<&[u8], u32> {
    // println!("alt");
    alt((parse_length_1_byte, parse_length_2_bytes, parse_length_3_bytes))(input)
}

fn parse_blob(input: &[u8]) -> IResult<&[u8], JsonValue> {
    // println!("parse blob");
    let (_, length) = peek(parse_length)(input)?;
    // println!("length {}", length);
    length_value(parse_length, map(rest, |x: &[u8]| JsonValue::String(hex::encode(x))))(input)
}
...
fn parse_amount(input: &[u8]) -> IResult<&[u8], JsonValue> {
    map(be_u64, |n: u64| JsonValue::String((n ^ 0x4000000000000000).to_string().into()))(input)
}

fn parse_account_id(input: &[u8]) -> IResult<&[u8], JsonValue> {
    map(preceded(take(1usize), take(20usize)), |address: &[u8]| JsonValue::String(ripple_address_codec::encode_account_id(<&[u8; 20]>::try_from(address).unwrap())))(input)
}
...
fn parse_field(input: &[u8]) -> IResult<&[u8], (String, JsonValue)> {
    ...
    pair(value(field_name.clone(), success(0)), match type_str {
        "UInt16" => parse_uint16,
        "UInt32" => parse_uint32,
        "UInt64" => parse_uint64,
